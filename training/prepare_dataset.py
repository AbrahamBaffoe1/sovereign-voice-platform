...
from training.common.manifest import SpeechRecord, dataset_fingerprint, file_sha256, normalize_transcript, stable_partition, write_jsonl
...
def _resolve_policy(args: argparse.Namespace) -> tuple[str, LanguageTrainingProfile | None, float, float]:
    ...
    if profile:
        if args.language and args.language != profile.code:
            raise SystemExit(f"--language {args.language!r} does not match profile language {profile.code!r}")
        return (
            profile.code,
            profile,
    ...
    return args.language, None, args.min_seconds if args.min_seconds is not None else 0.5, args.max_seconds if args.max_seconds is not None else 20.0
...
def main() -> None:
    ...
    with args.csv.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for row_number, row in enumerate(reader, 2):
            ...
            if profile:
                if profile.corpus.split_unit == "speaker" and not speaker: reasons.append("missing_speaker")
                if profile.corpus.require_consent and not consent_attested: reasons.append("consent_not_attested")
                if profile.corpus.require_reviewed_transcript and not transcript_reviewed: reasons.append("transcript_not_reviewed")
                if "dialect" in profile.corpus.required_metadata and not dialect: reasons.append("missing_dialect")
                if "source_id" in profile.corpus.required_metadata and not source_id: reasons.append("missing_source_id")
            if reasons:
            ...
            if quality.channels != 1 and require_mono and not args.allow_multichannel: reasons.append("multichannel_audio")
            ...
            if quality.duration < min_seconds: reasons.append("too_short")
            if quality.duration > max_seconds: reasons.append("too_long")
            ...
            record = SpeechRecord(audio_filepath=str(audio_path), text=text, duration=quality.duration, speaker=speaker, language=language, sha256=digest, dialect=dialect, source_id=source_id, consent_attested=consent_attested if profile else None, transcript_reviewed=transcript_reviewed if profile else None, split=split)
            splits[split].append(record); accepted.append(record)
    ...
    inventory = {"language": language, "accepted": len(accepted), "rejected": len(rejected), "split_unit": profile.corpus.split_unit if profile else "audio", "characters": [{"char": char, "count": count} for char, count in char_counter.most_common()]}
    (args.output / "inventory.json").write_text(json.dumps(inventory, ensure_ascii=False, indent=2), encoding="utf-8")
    quality_report = build_quality_report(accepted).as_dict(); quality_report.update({"language": language, "rejected": len(rejected)})
    (args.output / "quality_report.json").write_text(json.dumps(quality_report, ensure_ascii=False, indent=2), encoding="utf-8")
    fingerprint = dataset_fingerprint(accepted)
    dataset_version = {"language": language, "dataset_id": f"{language}-{fingerprint[:16]}", "fingerprint_sha256": fingerprint, "profile": str(args.profile) if args.profile else None, "profile_sha256": file_sha256(args.profile) if args.profile else None, "accepted": len(accepted), "hours": quality_report["hours"]}
    (args.output / "dataset_version.json").write_text(json.dumps(dataset_version, ensure_ascii=False, indent=2), encoding="utf-8")
