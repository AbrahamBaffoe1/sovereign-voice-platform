...
def initialize_corpus(profile_path: Path, output: Path) -> None:
    ...
    (wavs / ".gitkeep").touch(exist_ok=True)

    metadata = output / "metadata.csv"
    if not metadata.exists():
        with metadata.open("w", encoding="utf-8", newline="") as handle:
            csv.writer(handle).writerow(BASE_COLUMNS)

    policy = {
        "language": profile.code,
        "name": profile.name,
        "iso639_3": profile.iso639_3,
        "script": profile.script,
        "required_metadata": list(profile.corpus.required_metadata),
        "require_consent": profile.corpus.require_consent,
        "require_reviewed_transcript": profile.corpus.require_reviewed_transcript,
        "split_unit": profile.corpus.split_unit,
        "duration_seconds": {
            "min": profile.corpus.min_seconds,
            "max": profile.corpus.max_seconds,
        },
        "profile": str(profile_path),
        "note": (
            "Do not mark transcript_reviewed=true until a human reviewer has checked the recording "
            "against the transcript. Do not mark consent_attested=true without collection evidence."
        ),
    }
    (output / "CORPUS_POLICY.json").write_text(
...
def main() -> None:
    ...
    args = parse_args()
    initialize_corpus(args.profile, args.output)
    print(args.output)
